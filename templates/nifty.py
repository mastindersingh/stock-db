    ...
    for ticker in tickers:
        try:
            ...
            if 'Adj Close' not in stock_data.columns:
                print(f"Adjusted Close data not available for {ticker}")
                continue

            print(f"Processing data for {ticker}...")
            CAGR, pe_ratio, roe, debt_to_equity = calculate_financial_metrics(stock, stock_data)
...
def calculate_financial_metrics(stock, stock_data):
    # Example implementation for CAGR
    ...
    # Calculating P/E Ratio, Return on Equity (ROE), and Debt to Equity Ratio
...
nifty_tickers = ['RELIANCE.NS', 'TCS.NS', 'INFY.NS']
